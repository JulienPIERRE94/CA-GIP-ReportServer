...
Variables can be centralised in [scripts/ssrs/ssrs-config.sample.ps1](../scripts/ssrs/ssrs-config.sample.ps1) (to be duplicated as `ssrs-config.ps1`, kept out of source control).
...
**Script**: [01-Audit-SSRS.ps1](../scripts/ssrs/01-Audit-SSRS.ps1) – read-only, low risk.
...
**Script**: [03-Restore-SSRS.ps1](../scripts/ssrs/03-Restore-SSRS.ps1)
...
**Script**: [08-Test-SSRSReports.ps1](../scripts/ssrs/08-Test-SSRSReports.ps1) – uses the SSRS URL Access endpoint, low risk.
...
These queries can be plugged into the existing governance Power BI dashboard ([powerbi/api-monitoring](../powerbi/api-monitoring/)).
...
|---|---|---|
| [04-Migrate-SSRSPermissions.ps1](../scripts/ssrs/04-Migrate-SSRSPermissions.ps1) | Export/import SSRS policies (folder/report ACL) as JSON | Uses dynamic SOAP type instantiation, fragile across module versions. AD principals must already exist on the target. |
| [05-Migrate-SSRSDataSources.ps1](../scripts/ssrs/05-Migrate-SSRSDataSources.ps1) | Recreate shared data sources on the target with optional connection-string remap (CSV) | Stored credentials cannot be exported in clear text. Datasources using Store mode must be re-authenticated manually after import. |
| [06-Migrate-SSRSReports.ps1](../scripts/ssrs/06-Migrate-SSRSReports.ps1) | Download / upload of the full folder tree | Download/upload primitives are reliable. The optional -RebindDataSources switch is approximate and should not be relied upon: rebinding is best done manually in the Web Portal. |
| [07-Migrate-SSRSSubscriptions.ps1](../scripts/ssrs/07-Migrate-SSRSSubscriptions.ps1) | Export subscriptions to JSON and re-import on the target | Highest risk. The JSON round-trip via Get-RsSubscription / Set-RsSubscription is not officially supported and may fail on complex subscriptions (data-driven, file share delivery, custom delivery extensions). For production, prefer letting the database restore handle the subscriptions. |
| [09-Switch-SSRSProduction.ps1](../scripts/ssrs/09-Switch-SSRSProduction.ps1) | Disable old subs, switch DNS, enable new subs, send the comm e-mail | DNS scripting depends on the local DNS server type (Microsoft DNS / Infoblox / Azure DNS). Treat the DNS step as a template and replace it with the procedure aligned with your infra team. |
